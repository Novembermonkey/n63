...
def todo_add(title,user_id):
    ...
    data = (title,None,TodoType.PERSONAL.value,user_id)
    cursor.execute(insert_todo_query,data)
    return Response('Todo successfully created')


@login_required
@is_admin
@commit
def change_role(username, change_to: UserRole):
    get_user_role_query = """select role from users where username = %s"""
    role = cursor.execute(get_user_role_query, (username, ))
    print(get_user_role_query)

    if change_to == role:
        return Response(f"User is already {role}")

    if change_to == 'admin' :
        change_role_query = """update users set role = 'admin' where username = %s"""
        cursor.execute(change_role_query, (username,))
        return Response("User promoted to admin")
    elif change_to == 'user':
        change_role_query = """update users set role = 'user' where username = %s"""
        cursor.execute(change_role_query, (username,))
